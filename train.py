...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # grid = None  # Placeholder variable for the grid used to shift the image
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if grid is None:
        #     # Generate a grid for the shift
        #     grid_y, grid_x = torch.meshgrid(torch.arange(0, image.shape[-2]), torch.arange(0, image.shape[-1]))
        #     grid_x = grid_x.to(image.device)
        #     grid_y = grid_y.to(image.device)
        #     _h, _w = image.shape[-2], image.shape[-1]
        #     dx = _w // 2 - viewpoint_cam.cx
        #     dy = _h // 2 - viewpoint_cam.cy
        #     grid_x = grid_x.float() + dx
        #     grid_y = grid_y.float() + dy
        #     # Normalize the grid values to [-1, 1]
        #     grid_x = 2.0 * grid_x / max(image.shape[-1] - 1, 1) - 1.0
        #     grid_y = 2.0 * grid_y / max(image.shape[-2] - 1, 1) - 1.0
        #     grid = torch.stack([grid_x, grid_y], dim=2)
        #     grid = grid.unsqueeze(0)

        # shifted_image = torch.nn.functional.grid_sample(
